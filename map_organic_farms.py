...
def get_most_recent_data():
    ...
    fn = os.path.join('data', 'OID_data.xlsx')
    open(fn, 'wb').write(r.content)
    
    return pd.read_excel(fn, engine = 'xlrd' , skiprows = [1,2])


def download_census_map():
    url = 'https://www2.census.gov/geo/tiger/GENZ2020/shp/cb_2020_us_tract_500k.zip'
    fn = os.path.join('data', url.split('/')[-1])
    #subprocess.run(['curl', 
...
if __name__ == '__main__':
    ...
    if not os.path.exists(os.path.join('data', 'cb_2020_us_tract_500k')):
    ...
    gdf = gpd.read_file(os.path.join('data', 'cb_2020_us_tract_500k'))
    df = get_most_recent_data()
    
    df = df.fillna('')
    
    
    basemap = gpd.read_file(os.path.join('data', 
    ...
    zip_df = pd.read_csv(
        os.path.join('data', "ZIP_TRACT_032019.csv"), dtype={'zip': str})
  ...
  (~farms['Mailing Address: State/Province'].str.contains(non_contig))]
   ...
   #yr =  now.year
    ...
    plt.show()
    farms.to_csv(os.path.join('data', "all_farms.csv"))
#%%
